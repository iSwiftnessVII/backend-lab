...
async function ensurePlantillasDocumentoEquiposTable() {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS plantillas_documento_equipos (
...
exports.listarPlantillasDocumentoEquipo = async (req, res) => {
  try {
    await ensurePlantillasDocumentoEquiposTable();
    const [rows] = await pool.query(
      `SELECT id, nombre, nombre_archivo, mime, size_bytes, usuario_id, fecha_subida
       FROM plantillas_documento_equipos
       ORDER BY fecha_subida DESC, id DESC`
    );
    return res.json(rows);
  } catch (err) {
    console.error('Error GET /equipos/documentos/plantillas:', err);
    return res.status(500).json({ message: 'Error listando plantillas' });
  }
};

exports.subirPlantillaDocumentoEquipo = async (req, res) => {
  try {
    await ensurePlantillasDocumentoEquiposTable();
    const file = req.file;
    ...
    const [result] = await pool.query(
      `INSERT INTO plantillas_documento_equipos (nombre, nombre_archivo, mime, size_bytes, archivo, usuario_id)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        nombre,
    ...
    console.error('Error POST /equipos/documentos/plantillas:', err);
    return res.status(500).json({ message: 'Error subiendo plantilla' });
  }
};

exports.eliminarPlantillaDocumentoEquipo = async (req, res) => {
  try {
    await ensurePlantillasDocumentoEquiposTable();
...
exports.generarDocumentoEquipoDesdePlantilla = async (req, res) => {
  try {
    await ensurePlantillasDocumentoEquiposTable();
    const id = Number(req.params.id);
    if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ message: 'ID inválido' });
    ...
    return exports.generarDocumentoEquipo(req, res);
  } catch (err) {
    console.error('Error POST /equipos/documentos/plantillas/:id/generar:', err);
